...
rawCommand.SetHandler((string[] args, bool copy) =>
{
...
aliasCommand.SetHandler((string[] args, bool copy) =>
{
    var value = string.Join(" ", args);
    var emojis = Emoji.Find(value);
...
emojifyCommand.SetHandler((string[] args, bool copy) =>
{
    var value = string.Join(" ", args);
...
demojifyCommand.SetHandler((string[] args, bool copy) =>
{
    var value = string.Join(" ", args);
...
exportCommand.SetHandler((string[] args, string format, bool copy) =>
{
    var value = string.Join(" ", args);
    var emojis = Emoji.Find(value);
    string result;

    if (!string.IsNullOrEmpty(format) && format.ToLower() == "yaml")
    {
        var serializer = new YamlDotNet.Serialization.SerializerBuilder().Build();
        result = serializer.Serialize(emojis);
    }
    else if (!string.IsNullOrEmpty(format) && format.ToLower() == "xml")
    {
    ...
    else if (!string.IsNullOrEmpty(format) && format.ToLower() == "toml")
    {
    ...
    if (copy)
        ClipboardService.SetText(result);
}, argument, formatOption, copyOption);
